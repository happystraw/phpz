...
/// The underlying PHP execution data structure
execute_data: *c.zend_execute_data,
...
pub fn init(execute_data: *c.zend_execute_data) ExecContext {
    return .{ .execute_data = execute_data };
}

/// Get the raw PHP execution data pointer.
///
/// Use this if you need to call PHP C API functions directly that require
/// the execution data structure.
///
/// Returns:
///   The underlying zend_execute_data pointer
pub inline fn raw(self: *ExecContext) *c.zend_execute_data {
...
/// Returns:
///   The argument count
pub inline fn argCount(self: *const ExecContext) u32 {
    return self.execute_data.This.u2.num_args;
}
...
pub fn this(self: *ExecContext) ?*c.zval {
    if (Zval.phpType(&self.execute_data.This) == c.IS_OBJECT) {
        return &self.execute_data.This;
    }
    return null;
}
...
pub fn thisObject(self: *ExecContext) ?*c.zend_object {
    return c.zend_get_this_object(self.execute_data);
}
...
pub fn scope(self: *const ExecContext) ?*c.zend_class_entry {
    return @ptrCast(self.execute_data.func.?.common.scope);
}
