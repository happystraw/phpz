...
fn makePhpModuleStartupFn(comptime module_name: [:0]const u8, comptime hook_fn: ?PhpHookFn, comptime ini_entries: ?[]const ini.Entry) ?*const fn (c_int, c_int) callconv(.c) c.zend_result {
    ...
    return struct {
        fn @"fn"(_: c_int, module_number: c_int) callconv(.c) c.zend_result {
            ...
            if (comptime ini_entries) |entries| {
                // PHP INI entries
                if (c.zend_register_ini_entries(&ini.entries(entries)[0], module_number) != c.SUCCESS) {
                    return c.FAILURE;
                }
            }
...
fn makePhpModuleShutdownFn(comptime hook_fn: ?PhpHookFn, comptime ini_entries: ?[]const ini.Entry) ?*const fn (c_int, c_int) callconv(.c) c.zend_result {
    if (hook_fn == null and ini_entries == null) return null;
    return struct {
        fn @"fn"(_: c_int, module_number: c_int) callconv(.c) c.zend_result {
            if (comptime ini_entries != null) {
...
inline fn createModuleEntry(comptime cfg: Config) ModuleEntry {
    ...
    entry.module_startup_func = makePhpModuleStartupFn(cfg.name, cfg.module_startup_fn, cfg.ini_entries);
    entry.module_shutdown_func = makePhpModuleShutdownFn(cfg.module_shutdown_fn, cfg.ini_entries);
