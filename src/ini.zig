...
pub fn get(name: [:0]const u8) ?[]const u8 {
    const str = c.zend_ini_string(name.ptr, @intCast(name.len), 0);
    return if (str) |s| s.*.val()[0..s.*.len] else null;
}

/// Read a global INI value as an i64.
pub fn getLong(name: [:0]const u8, default: i64) i64 {
    return c.zend_ini_long(name.ptr, @intCast(name.len), default);
}
