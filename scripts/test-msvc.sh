...
xwin=$tmp_dir/xwin
win_sdk=$tmp_dir/win_sdk
libc_file=$tmp_dir/libc.txt

case "$php_thread_safety" in
    nts)
        php_archive_name=php-devel-pack-${php_version}-nts-Win32-vs17-x64.zip
        ;;
    zts)
        php_archive_name=php-devel-pack-${php_version}-Win32-vs17-x64.zip
        ;;
    *)
        echo "usage: $0 [PHP_VERSION] [nts|zts]" >&2
        exit 2
        ;;
esac

php_cache_dir=$tmp_dir/php-${php_version}-${php_thread_safety}
php_sdk=$php_cache_dir/php-${php_version}-devel-vs17-x64
